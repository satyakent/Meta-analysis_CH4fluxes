...
log10rate=ggplot(flux_rates, aes(x = Pathway, y = Mean_rate, fill = Pathway)) +
  ...
  scale_fill_manual(values = c("lightblue", "royalblue", "mediumblue")) +
  geom_jitter(color = "grey39", size = 1.5, alpha = 0.3) +
  geom_text(aes(label = "n = 114", x = "Methanogenesis", y= 27), size=4) +
...
# There was a statistically significant difference in rate between the different pathways (χ2(2)=126.52, p = 0). Significant differences were observed between AMO and ANME, (Z = 7.89, adjusted p = 0) and between mgen and ANME (Z = -10.11, adjusted p = 0).
...
#calculate median and IQR for each Pathway
flux_rates_filtered %>%
  group_by(Pathway) %>%
  summarize(
